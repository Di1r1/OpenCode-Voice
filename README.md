...
- `/voice` auto-stop: recording continues while you speak and stops ~1.5 s after you fall silent (hard cap 60 s of audio). The recorder is stopped gracefully (SIGINT) so the WAV header stays valid. On slow RDP audio the capture may take longer than real time.
...
| `OPENCODE_VOICE_RECOGNIZED_LOG` | File where every recognized text is logged with source (`command`/`button`), backend, model, language and duration | `/tmp/opencode/voice-recognized.log` |
