...
> **`.opencode/` is generated locally.** `sync-plugin.sh` creates `voice-opencode-plugin/.opencode/plugins/index.ts` and `.opencode/tui/voice.ts` (it copies `src/index.ts` and rewrites imports). The folder is git-ignored — run the sync step after every clone and after editing `src/index.ts`.
...
└── .opencode/               # generated by sync-plugin.sh (git-ignored)
```
