...
- **Audio Recording**: `src/lib/recorder.ts` (`startPushToTalk`/`waitPushToTalkEnd`/`pttFileSize`/`recoverMic`; arecord/ffmpeg, mono 16 kHz S16_LE, pinned `OPENCODE_VOICE_SOURCE`). `/voice` records a fixed 30 s window: start the recorder, wait for it to finish, then transcribe. Recordings go to RAM (`/dev/shm/opencode-voice`, tmpfs) and are auto-deleted after `OPENCODE_VOICE_RETAIN_SECONDS` (default 300 s).
...
- **Hook trick**: `command.execute.before` suppresses the markdown command template by setting `output.parts.length = 0` and pushing `{type:"text",text:"\n"}`. OpenCode always calls the model afterwards, so this is the only way to avoid the raw `$ARGUMENTS` template (an empty parts array triggers an API error). On failure paths the hook throws instead, so no empty prompt is sent (OpenCode logs it as an error — known SDK limitation).
