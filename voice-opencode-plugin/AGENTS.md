...
- **Testing**: hermetic `pytest` (`stt-server/tests/`, 42 tests, no mic/model: `pip install -r stt-server/requirements-dev.txt && pytest`), TS tests `npm test` (`node --experimental-strip-types --test`, 21 cases: `stripNonSpeech` from `src/lib/text.ts` + whisper path/model resolution from `src/lib/whisper.ts`), `npm run typecheck`, `bash sync-plugin.sh --check`; CI in `.github/workflows/ci.yml`. Plus live `/voice` checks through the TUI. Push-to-talk requires a real mic/PulseAudio.
