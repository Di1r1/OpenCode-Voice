...
- **Audio Recording**: `src/lib/recorder.ts` (`startPushToTalk`/`waitPushToTalkEnd`/`stopPushToTalk`/`pttFileSize`; arecord/ffmpeg, mono 16 kHz S16_LE). Recording is non-blocking: `startPushToTalk` spawns a detached process and returns immediately; a background task waits for it and then transcribes.

## Commands (`/voice`)

- `/voice` - Non-blocking toggle: the first call starts a background recording (beep + live TUI timer), the second stops it; a background task then transcribes and inserts the text into the prompt. Both start and stop **throw** to abort the command, so OpenCode sends no empty prompt (a thrown hook error shows a service toast). Short beeps play on start/stop (`beep()` in `src/lib/beep.ts`, aplay/paplay/ffplay). While recording, the server plugin writes `/tmp/opencode/voice-status.json` and the TUI plugin (`.opencode/tui/voice.ts`) polls it and shows a live toast timer. Service markers (`[музыка]`, `(смех)`, `♪`) are stripped (`stripNonSpeech` in `src/lib/stt.ts`, `_strip_non_speech` in `stt_server.py`).
