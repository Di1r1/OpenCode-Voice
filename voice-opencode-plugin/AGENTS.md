...
- **Testing**: hermetic `pytest` (`stt-server/tests/`, 44 tests, no mic/model: `pip install -r stt-server/requirements-dev.txt && pytest`), TS tests `npm test` (`node --experimental-strip-types --test`, 24 cases: `stripNonSpeech` from `src/lib/text.ts` + whisper path/model resolution from `src/lib/whisper.ts`), `npm run typecheck`, `bash sync-plugin.sh --check`; CI in `.github/workflows/ci.yml`. The `stripNonSpeech` cases are shared: `shared/strip-cases.json` is read by both `npm test` and `pytest` (Python parity was missing before). Plus live `/voice` checks through the TUI. Push-to-talk requires a real mic/PulseAudio.
