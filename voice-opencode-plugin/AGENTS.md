...
- **STT Transcription**: `src/lib/stt.ts` (`openai` SDK for `api` backend; `faster-whisper`/whisper.cpp for `local`). Paths are resolved by `src/lib/whisper.ts` (same logic mirrored in `stt_server.py`): base dir `OPENCODE_VOICE_HOME` (`~/.local/share/opencode-voice`), CLI/model auto-detected in `OPENCODE_VOICE_WHISPER_DIR` (`<home>/whisper`), CUDA dirs discovered via `cuda-*` globs plus `CUDA_HOME`/`CUDA_PATH` (no hardcoded toolkit version). Model size: `WHISPER_MODEL`/`WHISPER_CPP_MODEL_SIZE` override, otherwise `medium` on GPU and `small` on CPU. Device selection via `OPENCODE_VOICE_DEVICE` (`auto`|`gpu`|`cpu`) or `/voice device`: `auto` uses whisper.cpp+CUDA when `libcuda` is present, otherwise falls back to `faster-whisper` on CPU. `OPENCODE_VOICE_STT_BACKEND=whispercpp|faster-whisper` is kept as an alias. Shared values (non-speech markers, silence defaults, whisper.cpp extra flags, per-device model sizes) live in `shared/stt-spec.json` — TS and Python both read it, so there are no duplicated constants.
- **Audio Recording**: `src/lib/recorder.ts` (`startPushToTalk`/`waitPushToTalkAuto`/`waitPushToTalkEnd`/`pttFileSize`/`recoverMic`; arecord/ffmpeg, mono 16 kHz S16_LE, pinned `OPENCODE_VOICE_SOURCE`). `/voice` records until ~1.5 s of silence after speech (hard cap `OPENCODE_VOICE_MAX_RECORD_SECONDS`, default 300 s) and stops the recorder gracefully (SIGINT) so the WAV header is always finalized — the old SIGKILL truncated long recordings and left a stale header. Recordings go to RAM (`/dev/shm/opencode-voice`, tmpfs) and are auto-deleted after `OPENCODE_VOICE_RETAIN_SECONDS` (default 300 s).
- **Server & autostart**: `src/lib/server-launcher.ts` starts `stt-server/stt_server.py` on plugin load (`ensureSttServer`) and keeps it alive (`startServerWatchdog`). Env: `OPENCODE_VOICE_SERVER` (default `1`), `_SERVER_SCRIPT`, `_PORT` (8765), `_SERVER_LOG` (`/tmp/opencode/stt_server.log`), `_SERVER_WATCHDOG_MS` (120000). Auto-sets `PULSE_SERVER=unix:/mnt/wslg/PulseServer` when that socket exists. The server binds `127.0.0.1` and answers CORS only for local origins; optional `OPENCODE_VOICE_TOKEN` protects every endpoint except `/health`. Overload guard (server): `OPENCODE_VOICE_MAX_UPLOAD_MB` (25 → `413`), `_MAX_AUDIO_SECONDS` (300 → `400`), `_MAX_CONCURRENT` (1 → `429`), `_TRANSCRIBE_TIMEOUT` (300 → `504`), `_RATE_LIMIT` (60/min per IP → `429`), `_PURGE_INTERVAL` (600 s); POSTs with a foreign `Origin` get `403`.
...
- `backend`/`lang`/`device` choices persist to `OPENCODE_VOICE_STATE_FILE` (default `~/.config/opencode-voice/state.json`) via `src/lib/state.ts`; env vars still win at startup.
...
- **Testing**: hermetic `pytest` (`stt-server/tests/`, 44 tests, no mic/model: `pip install -r stt-server/requirements-dev.txt && pytest`); Node suite `npm test` (`node --experimental-strip-types --test`, **41 tests**: `stripNonSpeech` from `src/lib/text.ts`, whisper path/model resolution from `src/lib/whisper.ts`, state persistence (`src/lib/state.ts`), plus hermetic E2E); `npm run typecheck`; `bash sync-plugin.sh --check`. E2E (`test/e2e-plugin.test.mjs`, `test/e2e-server.test.mjs`) runs the real recorder/STT code with a fake Bun-`$` (`test/helpers/fake-bash.mjs`), a stub recorder (`OPENCODE_VOICE_RECORDER_BIN`) and a stub whisper-cli — no mic/model/network. The server E2E spawns the actual server over HTTP and **skips itself when Flask is absent** (Node-only CI job), while the server job runs it explicitly. Node cannot resolve extensionless imports, so relative imports inside `src/lib/**` use explicit `.ts` extensions (`allowImportingTsExtensions` in `tsconfig.json`). The server kills stray recorders on startup; tests disable it with `OPENCODE_VOICE_STALE_CLEANUP=0`. `stripNonSpeech` cases are shared (`shared/strip-cases.json`, read by both `npm test` and `pytest`). Plus live `/voice` checks through the TUI; push-to-talk requires a real mic/PulseAudio.
...
- **Subagents**: `voice-builder`, `voice-stt` — optional **local-only** dev agents (`.opencode/agents/`, git-ignored); they are not part of the published package. Also loadable as `@voice-builder` / `@voice-stt` in the TUI.
