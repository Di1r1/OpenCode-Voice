...
- **Testing**: hermetic `pytest` (`stt-server/tests/`, 37 tests, no mic/model: `pip install -r stt-server/requirements-dev.txt && pytest`), TS tests `npm test` (`node --experimental-strip-types --test`, 12 cases for `stripNonSpeech` from `src/lib/text.ts`), `npm run typecheck`, `bash sync-plugin.sh --check`; CI in `.github/workflows/ci.yml`. Plus live `/voice` checks through the TUI. Push-to-talk requires a real mic/PulseAudio.
