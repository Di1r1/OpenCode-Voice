...
4. Опционально: **удерживайте `Alt+Z`** на странице OpenCode — запись идёт, пока держите клавиши; отпустите — стоп и распознавание (работает только на странице OpenCode). Комбинацию можно сменить в popup расширения (`Alt+Z`, `Ctrl+Shift+Z`, `Alt+Q`, `F9`); сначала модификатор, затем клавиша, отпускать можно в любом порядке (Escape — отмена).
