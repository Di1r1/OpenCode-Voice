...
| `OPENCODE_VOICE_HOST` | адрес привязки STT-сервера; по умолчанию только localhost. `0.0.0.0` — только если нужен доступ по сети (аутентификации нет) | `127.0.0.1` |
| `OPENCODE_VOICE_SERVER_WATCHDOG_MS` | период проверки/перезапуска, `0` = выкл | `120000` |

**Безопасность:** STT-сервер слушает только `127.0.0.1` и отдаёт CORS лишь локальным origin (UI OpenCode, расширение). **Аутентификации нет**: если открыть его наружу (`--host 0.0.0.0` / `OPENCODE_VOICE_HOST=0.0.0.0`), любой в сети сможет писать с твоего микрофона и читать расшифровки.
