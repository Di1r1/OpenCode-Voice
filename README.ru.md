...
4. Опционально: **удерживайте `Alt+Z`** на странице OpenCode — запись идёт, пока держите клавиши; отпустите — стоп и распознавание (работает только на странице OpenCode).
...
TUI: хоткей `<leader>v` (leader по умолчанию `ctrl+x`) запускает push-to-talk. Веб-интерфейс: кнопка 🎤 из расширения либо **удержание `Alt+Z`** на странице OpenCode (push-to-talk).
